...
const Login = () => {
    ...
    const handleLogin = () => {
      axios.post(`http://192.168.1.157:5000/login`, { username, password })
      .then((res) => {
    ...
    const handleRegister = () => {
      axios.post(`http://192.168.1.157:5000/register`, { username, password })
      .catch((error) => {
    ...
    if (status == "Login success.") {
      return (
        <View style={styles.container}>
          <Text>{status}</Text>
          <Link to="/"><Text style={styles.buttonStyle}>Continue to app</Text></Link>
      </View>)
    } else {
    return (
      <View style={styles.container}>
    ...
    />
      ...
      value={password}
    />
    <Text style={styles.buttonStyle} onPress={() => handleRegister()}>REGISTER</Text>
    <Text style={styles.buttonStyle} onPress={() => handleLogin()}>LOG IN</Text>
      </View>
    );
  }}
